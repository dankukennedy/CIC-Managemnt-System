...
class RegisterController
{
    ...
    {
      $register_query="INSERT INTO managers(fName,lastName,otherName,contact,nia,field,qualification,email,gender,working,cicName,cicGps,region,subDivision,securityQsn,answer,dateOfBirth,password,profile_img	) VALUES ('$fName','$lastName','$otherName','$contact','$nia','$field','$qualification','$email','$gender','$working','$cicName','$cicGps','$region','$subDivision','$securityQsn','$answer','$dateOfBirth','$password','$profile_img	' )";
    ...
    //checking Whether contact entered existed    
     ...
     {
         $checkManager="SELECT contact FROM managers WHERE contact='$contact' LIMIT 1";
...
//Checking whether the email entered existed
    ...
    {
        $checkManager="SELECT email FROM managers WHERE email='$email' LIMIT 1";
        $result=$this->conn->query($checkManager);
        if ($result->num_rows > 0) {            
        return true;
        }else {
